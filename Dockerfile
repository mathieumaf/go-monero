...
FROM $RUNTIME_IMAGE
        ...
        # ca-certificates lets the CLI reach a daemon/wallet RPC endpoint over
        # HTTPS; the rest of alpine (shell, apk) stays available for debugging.
        RUN apk add --no-cache ca-certificates && \
                adduser -D -H -u 65532 nonroot

        COPY --from=builder /workspace/monero /monero
